  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        ...
        if (!company_id) {
          setCustomersError("Company ID not found. Please log in again.");
          setLoadingCustomers(false);
          return;
        }

        const response = await fetch('http://localhost/Trashroutefinal1/Trashroutefinal/TrashRouteBackend/Company/routemapaccess.php', {
          method: 'POST',
          headers: {
        ...
        if (result.success) {
          ...
          if (result.data.length > 0) {
            const locations = result.data.map(customer => ({
          ...
          setCustomersError(result.error || 'Failed to fetch customer data');
        }
      } catch (err) {
